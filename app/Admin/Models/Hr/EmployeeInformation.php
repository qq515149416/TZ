...
class EmployeeInformation extends Model
{
    ...
    */
    ...
    public function editEmployee($data){
    	if($data && $data['id']+0){
    		// 查找对应的数据
    		$editinfor = $this->find($data['id']);
    		// 对要修改的数据进行一一赋值
    		$editinfor->admin_users_id = $data['admin_users_id'];
    		$editinfor->fullname = $data['fullname'];
    		$editinfor->sex = $data['sex'];
    		$editinfor->age = $data['age'];
    		$editinfor->department = $data['department'];
    		$editinfor->job = $data['job'];
    		$editinfor->entrytime = $data['entrytime'];
    		$editinfor->work_number = $data['work_number'];
    		$editinfor->phone = $data['phone'];
    		$editinfor->QQ = $data['QQ'];
    		$editinfor->wechat = $data['wechat'];
    		$editinfor->email = $data['email'];
    		$editinfor->dimission = $data['dimission'];
    		$editinfor->education = $data['education'];
    		$editinfor->train = $data['train'];
    		$editinfor->work = $data['work'];
    		$editinfor->skill = $data['skill'];
    		$editinfor->detailed = $data['detailed'];
    		$editinfor->family = $data['family'];
    		$editinfor->note = $data['note'];
    		// 进行修改
    		$row = $edit->save();
    		if($row != false){
				// 修改数据成功
    ...
    public function employeeDetailed($param){
    	if($param['detailed_id']+0){
    		$result = $this->where($param)->get(['education','train','work','skill','detailed','family']);
    		if(!$result->isEmpty()){
    			$return['data'] = $result;
	    		$return['code'] = 1;
	    		$return['msg'] = '获取信息成功！';
