...
*
*/
class RemoveReasonController extends Controller
{
	...
	protected function grid()
	{
		...
		return Admin::grid(BusinessModel::class, function (Grid $grid) {
			...
			$grid->column('sales_name', '所属业务员');
			$grid->column('machine_num','机器编号')->display(function () {
				$detail = json_decode($this->resource_detail,true);
				//dd($detail);
				if (isset($detail['machine_num'])) {
					$machine_num = $detail['machine_num'];
				}else{
					$machine_num = '机器编号信息错误';
				}
				
				if (isset($detail['business'])) {
					$business_type = $detail['business'];
				}else{
					$business_type = '业务类型错误';
				}
				
				return $business_type .' : '.$machine_num;
			})->sortable();
			// $grid->column('business_type','类型')->display(function () {
			// 	$detail = json_decode($this->resource_detail,true);
			// 	$business_type = $detail['business'];
			// 	return $business_type;
			// })->sortable();
			$grid->column('money','单价');
			$grid->column('start_time','开始日期');
			$grid->column('endding_time','结束日期');
			$grid->column('remove_status','下架进度')->display(function () {
				$status = [
					3 => '清空下架中',
					4 => '下架完毕',
				];
				return $status[$this->remove_status];
			});
			$grid->column('remove_reason','下架原因');
			$grid->column('check_note','审核备注');
