...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(12), __webpack_require__(72).setImmediate))
...
!function (a, b) {
  ...
  var c = [],
    ...
    return this.length;
  }, n.fn.andSelf = n.fn.addBack, "function" == "function" && __webpack_require__(165) && !(__WEBPACK_AMD_DEFINE_ARRAY__ = [], __WEBPACK_AMD_DEFINE_RESULT__ = (function () {
    return n;
  }).apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__),
...
if (document.querySelector("#registered")) {
    ...
    new Vue({
        ...
        methods: {
            s_salesman: function s_salesman() {
                document.querySelector(".select").style.display = "block";
                document.body.style.overflow = "hidden";
            },
            close_salesman: function close_salesman() {
            ...
            //提示框
            textTip: function textTip(str, t, callBack) {
                t = t || 2000;
            ...
            EmailTextTip: function EmailTextTip(str) {
            ...
            registered: function registered() {
                ...
                $.post("/auth/registerByEmail", {
                    ...
                    if (data.code == 1) {
                        ...
                        _this.token = "";
                        _this.get_token.verification_code = "";
                        _this.msg_qq = "";
                        _this.msg_phone = "";
                        _this.nickname = "";
                        location.href = "/tz/member92019.html#/";
                    } else {
                        _this.textTip(data.msg);
                        _this.switchCaptchaSrc();
                    }
                });
            },
            ...
            getToken: function getToken() {
                ...
                $.post("/auth/sendEmailCode?r=" + Math.floor(Math.random() * 10000), {
                    ...
                    if (data.code == 1) {
                        _this2.EmailTextTip("验证码已发送到你的邮箱，请留意查收");
                        _this2.get_token.wait_time = 60;
                        $("#get-verification-code").val("重新获取（" + _this2.get_token.wait_time + ")");
                        waitSendVerificationCode(_this2.get_token.wait_time, function () {
                            if (_this2.get_token.wait_time > 0) {
                                _this2.get_token.wait_time--;
                                $("#get-verification-code").val("重新获取（" + _this2.get_token.wait_time + ")");
                            } else {
                        ...
                        _this2.textTip(data.msg);
                        _this2.switchCaptchaSrc();
                    }
                });
            },
            switchCaptchaSrc: function switchCaptchaSrc() {
                var _this3 = this;

                $.get("/verification_code", function (data) {
                    if (data.code == 1) {
                        _this3.get_token.captcha_src = data.data.src;
                    }
                });
            }
        },
        mounted: function mounted() {
            ...
            $.get("/verification_code", function (data) {
                if (data.code == 1) {
                    ...
                    _this4.switchCaptchaSrc();
                    _this4.salesmans = [];
                    $.get("/auth/getAllSalesman", function (res) {
                        if (res.code == 1) {
                            _this4.salesmans = res.data;
                            console.log(_this4.salesmans);
                            for (var i = 0; i < _this4.salesmans.length; i++) {
                                //循环数组中的所有的元素
                                //获得盒子,向盒子里面循环添加li,li里面附带a标签 
                                $("#salesman-form").append("<div><input type='radio' name='salesman' id='" + _this4.salesmans[i]['id'] + "' value='" + _this4.salesmans[i]['name'] + "'><label></label>" + _this4.salesmans[i]['name'] + "</div>");
    ...
    new Vue({
        ...
        methods: {
            switchCaptchaSrc: function switchCaptchaSrc() {
                ...
                $.get("/verification_code", function (data) {
                    if (data.code == 1) {
                        _this.captcha_src = data.data.src;
                    }
                });
            },
            textTip: function textTip(str, callBack) {
                // t = t || 2000;
                var dom = document.createElement("p");
                dom.setAttribute('class', 'text-tip');
                document.body.appendChild(dom);
            ...
            login: function login() {
                ...
                if (this.email == "" || this.password == "") {
                    ...
                    $.post("/auth/login", {
                        ...
                        if (data.code == 1) {
                            _this2.textTip("登陆成功");
                            if (_this2.rememberChecked) {} else {}
                            location.href = "/wap/logging";
                        } else {
                            _this2.switchCaptchaSrc();

                            _this2.error = true;
                            _this2.error_msg = data.msg;
                            _this2.textTip(data.msg);
                        }
                    });
        ...
        mounted: function mounted() {
            ...
            $.get("/verification_code", function (data) {
                if (data.code == 1) {
                    _this3.captcha_src = data.data.src;
                }
            });
