...
class LoginController extends Controller
{
    ...
    public function login(LoginRequest $request)
    {
        ...
        if ($userExists) {
            ...
            if ($loginInfo) {
                ...
                $login_old_oa = $this->loginByVerZero($res['login_name'], $res['password']);
                if ($login_old_oa) {
                    return tz_ajax_echo([], '登录成功', 1);//登录成功
                }
     ...
     * @return bool
     */
    protected function loginByVerZero($loginName, $password)
    {
        $model = new TzUser();
        $res   = $model->loginByVerZero($loginName, $password);
        if ($res) {
            return Auth::attempt(['name' => $loginName, 'password' => $password]);
        } else {
            return false;
    ...
    public function loginCheck()
    {
        ...
        $userData = Auth::user();//获取用户登陆数据
        return tz_ajax_echo($userData, '已登录', 1);
    }
