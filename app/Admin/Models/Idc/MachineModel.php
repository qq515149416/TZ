...
class MachineModel extends Model
{
	...
	public function editMachine($editdata){
		...
		if($machine->used_status == 2){//当机器的状态为使用中时，更改业务里面的resource_detail字段
			$business = DB::table('tz_business')->where(['business_number'=>$machine->own_business])->whereBetween('business_status',[0,1])->select('id','business_number','machine_number','resource_detail','remove_status')->first();
			if(!empty($business)){
